...
- **文档路径模板变量**:「默认文档路径」设置支持模板变量,文档可按日期自动归档而非全部堆在同一目录。支持 `{{date}}`(→ 2026-06-10)、`{{year}}`、`{{month}}`、`{{day}}`、`{{title}}` 简单变量及 `{{now | date "YYYY-MM-DD"}}` 自定义日期格式化管道;路径自动规范化(去尾斜杠、合并连续斜杠)。模板中显式使用 `{{title}}` 时不重复追加,否则标题仍自动拼接末尾。

> 新增 `domain/template.ts`(`resolveDocPath` + `formatGoStyleDate`)+ `EventEditor.svelte` 调用点替换原始字符串拼接+`index.ts` 设置项 description 补充变量说明。测试 540 → **555**。
...
- **时间分布占比环形卡**:复盘页概览区新增「时间分布」环形卡(conic-gradient 环),按**数量占比**展示事项构成,回答「这段时间事项都集中在哪类」。单环 + tab 切换**状态 / 优先级 / 标签**三个分组维度,跟随顶部已有周 / 月 / 年 / 全部选择器。状态(待办 / 进行中 / 已完成 / 已取消)、优先级(高 / 中 / 低)显全部固定段(含占比 0 的段);标签维度按出现次数 Top 8 聚合、超出归「其他」段殿后,只显非零段。环中心显总数 + 单位(状态 / 优先级「件」、标签「次」),图例每段显色点 + 名称 + `数量 (百分比%)`。与现有「任务分布」横向条形(绝对量 + 完成率)形成「占比视角 vs 绝对量视角」互补。

> 纯函数 `domain/stats.ts#computeProportion`(三维度占比聚合 + `TAG_PALETTE` 调色板)+ 组件 `components/_review/ProportionDonut.svelte`(conic-gradient 环 + tab 切维度 + 图例,`prefers-reduced-motion` 禁动画、全 `--chx-*` token)+ `ReviewView.svelte` 集成(`$:` 响应顶部周期选择器)。零数据模型变更、零新依赖、纯前端读 `eventsStore`。测试 526 → **540**。
...
- **同日法定假日与纪念节日并存显示**:此前一天同时命中法定节假日(如端午节调休放假日)和纪念节日(如父亲节,6 月第 3 个周日)时,法定优先、纪念被丢弃,导致 6-21 只显示「夏至 + 端午节」而看不到父亲节——与顶部「本月节假日」栏(已合并法定+纪念)不一致。现拆分为法定 / 纪念两条独立渲染轨并存:法定保留「休/班」角标 + 背景色 + 节日名,纪念作为独立一行显示。

> `DayCell` 新增 `observance` prop(纪念节日),与 `holiday`(法定)解耦;`MonthGrid` / `WeekView` 各自构造 `holidayByDate`(`getHolidayForDate`)+ `observanceByDate`(`getObservanceForDate`)两个 map 分别传入。月视图、周视图同步生效。测试 511 → **526**(新增 DayCell 并存用例)。
