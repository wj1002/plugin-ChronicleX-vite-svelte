...
- **v1.11.1** (2026-06-18): **Fix — settings priority color swatches overlapping** — the "Colors" section's high / normal / low preset swatches previously used `padding + negative margin` to expand the touch target; a later `padding: 0` left the negative margin behind, pulling adjacent swatches together and eating the flex gap, so they overlapped. Removed the negative margin, bumped `gap` 8→10px, added `box-sizing`. See [CHANGELOG](CHANGELOG.md). Tests 650
